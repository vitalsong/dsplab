...
void QwtDynGridLayout::setGeometry(const QRect& rect)
{
    ...
    int index = 0;
    for (QList<QLayoutItem*>::const_iterator it = m_data->itemList.constBegin(); it != m_data->itemList.constEnd();
         ++it) {
 ...
 */
void QwtDynGridLayout::stretchGrid(const QRect& rect, uint numColumns, QVector<int>& rowHeight,
